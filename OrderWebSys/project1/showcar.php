                ...
                if(isset($_COOKIE['currentnumitem']))
                {
                    ...
                    for($a=1 ; $a<=$numitem ;$a++)
                    {
                        ...
                        if(isset($arr) && $arr[0]!='0')
                        {
                            ...
                            echo "<p>"."單價: ".$arr[3]." 元  "."選購份數: ".$arr[2]." 份 </p></a>";
                         
                            $dbt='dbt'.$a ;//每次迴圈所產生的delete BUT id編號
                            echo "<a  id='$dbt' class='button' data-icon='delete'>移除</a>";
                            $reselect='reselect'.' '.$a ;//每次迴圈所產生的select id編號
                            $q=(int)$arr[2];//宣告q=選購份數                         
